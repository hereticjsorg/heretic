        ...
        switch (answers.action) {
            case "Remove module":
                const modulesList = fs
                    .readdirSync(path.resolve(__dirname, "../../site/modules"))
                    .filter((p) => !p.match(/^\./));
            ...
            case "Add language":
                ...
                await binUtils.addLanguage(
                    addLanguageAnswers.id,
                    addLanguageAnswers.name,
                );
                break;
            case "Remove language":
                const languagesConfigPath = path.resolve(
                    __dirname,
                    "../../site/etc/languages.json",
                );
                if (!fs.existsSync(languagesConfigPath)) {
                    binUtils.log(
                        `The "site/etc/languages.json" file is missing`,
                        {
                            error: true,
                            noDate: true,
                        },
