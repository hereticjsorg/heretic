...
(async () => {
    ...
    const dirPath = config.directories.tmp
        ? path.resolve(__dirname, config.directories.tmp, updateId)
        : path.join(os.tmpdir(), updateId);
    ...
    try {
        ...
        const rootFiles = (await fs.readdir(path.join(dirPath, "root"))).filter(
            (f) => f !== "package.json",
        );
        ...
        if (options["restart-pm2"]) {
            try {
                ...
                binUtils.log(
                    "Could not restart PM2 process. Is PM2 installed?",
                    {
                        error: true,
                    },
                );
        ...
        if (!options["npm-install"]) {
            binUtils.log(
                "Please run 'npm run install-modules' in order to update NPM modules",
            );
        }
        if (!options["rebuild-dev"] && !options["rebuild-production"]) {
            binUtils.log(
                "Please run 'npm run build' in order to rebuild Heretic",
            );
        }
        if (!options["restart-pm2"]) {
            binUtils.log(
                `Please run 'pm2 restart ${config.id}' in order to restart Heretic`,
            );
        }
    } catch (e) {
        binUtils.log(e.message, {
