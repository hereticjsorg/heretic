...
export default class {
    async onCreate(input, out) {
        ...
        if (input.admin) {
            await import(
                /* webpackChunkName: "hfxbulk-admin" */ "./style-admin.scss"
            );
        } else {
            await import(
                /* webpackChunkName: "hfxbulk-frontend" */ "./style-frontend.scss"
            );
        }
    ...
    async show() {
        ...
        await this.utils.waitForComponent(
            `bulkUpdateModal_hf_${this.input.id}`,
        );
    ...
    async bulkUpdateData(id) {
        switch (this.input.columnData[id].type) {
            case "select":
                const bulkItemEditSelectValues = this.input.columnData[
                    id
                ].options.map((i) => ({
                    value: i.value,
                    label: i.label,
    ...
    async setBulkItemTabs() {
        if (this.input.tabs && this.input.tabs.length) {
            await this.utils.waitForComponent(
                `bulkItem_hf_${this.input.id}_tabs`,
            );
    ...
    async bulkItemNew(e) {
        ...
        const bulkItemModal = this.getComponent(
            `bulkItemModal_hf_${this.input.id}`,
        );
        this.setState("bulkItemUID", null);
        bulkItemModal
            .setActive(true)
        ...
        await this.utils.waitForElement(
            `bulkItem_hf_${this.input.id}_select_id`,
        );
        await this.setBulkItemTabs();
        document
            .getElementById(`bulkItem_hf_${this.input.id}_select_id`)
            .focus();
    }
                ...
                try {
                    ...
                    await axios({
                        ...
                        data: {
                            ...
                            bulkItems,
                            filters: this.input.filters.filter(
                                (i) => i.enabled,
                            ),
    ...
    async bulkSetValue(id, value) {
        switch (this.input.columnData[id].type) {
            case "date":
                await this.utils.waitForComponent(
                    `bulkItem_hf_${this.input.id}_hcalendar`,
                );
                this.getComponent(
                    `bulkItem_hf_${this.input.id}_hcalendar`,
