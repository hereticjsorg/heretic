...
export default class {
    initValidation(input = this.input) {
        ...
        if (input.data.getValidationSchema) {
            this.formValidator = new FormValidator(
                input.data.getValidationSchema(),
                this.fieldsFlat,
            );
        }
        if (process.browser) {
    ...
    async onCreate(input, out) {
        ...
        this.state = {
            ...
            modeChangeAllowed:
                input.data.isModeChangeAllowed &&
                input.data.isModeChangeAllowed(),
            data: {},
            errors: {},
            errorMessage: null,
        ...
        if (input.admin) {
            await import(
                /* webpackChunkName: "hform-admin" */ "./style-admin.scss"
            );
        } else {
            await import(
                /* webpackChunkName: "hform-frontend" */ "./style-frontend.scss"
            );
        }
    }
        ...
        for (const k of Object.keys(this.fieldsFlat)) {
            ...
            if (field.autoFocus) {
                const component = this.getComponent(
                    `hr_hf_f_${field.id}_${this.state.mode}`,
                );
    ...
    async onMount() {
        ...
        window.addEventListener("click", (e) => {
            const dbAddTabElement = document.getElementById(
                `${this.input.id}_dm_addTab`,
            );
            ...
            if (this.state.historyActionsDropdownOpen) {
                const historyActionButton = document.querySelector(
                    `[data-id="${this.state.historyActionsDropdownOpen}"]`,
                );
                if (
                    historyActionButton &&
                    !historyActionButton.contains(e.target)
                ) {
        ...
        for (const id of this.fieldIds) {
            if (serializableTypes.indexOf(this.fieldsFlat[id].type) > -1) {
                const fieldComponent = this.getComponent(
                    `hr_hf_f_${id}_${this.state.mode}`,
                );
                if (fieldComponent) {
                    data[id] = fieldComponent.getValue();
                    if (this.fieldsFlat[id].type === "password") {
                        const { repeat, value } = data[id];
                        data[id] = value;
                        this.passwordRepeat[this.state.activeTab] =
                            this.passwordRepeat[this.state.activeTab] || {};
            ...
            if (serializableTypes.indexOf(this.fieldsFlat[id].type) > -1) {
                ...
                if (this.fieldsFlat[id].type === "keyValue") {
                    for (const item of serialized[id] || []) {
                        const currentKeyValueItem =
                            this.state.keyValueData.find(
                                (i) => i.id === item.id,
                            ) || {};
                        ...
                        switch (currentKeyValueItem.type) {
                            ...
                            case "list":
                                const valueItem =
                                    currentKeyValueItem.items.find(
                                        (i) => i.id === item.value,
                                    );
                                item.valueLabel = valueItem
                                    ? valueItem.label
                                    : valueItem;
                                break;
                            default:
                ...
                if (fieldComponent) {
                    fieldComponent.setValue(
                        typeof serialized[id] === "undefined"
                            ? null
                            : serialized[id],
                    );
                }
        ...
        for (const tab of this.state.tabs) {
            if (this.formValidator) {
                ...
                for (const id of Object.keys(data[tab])) {
                    if (
                        this.fieldsFlat[id] &&
                        this.fieldsFlat[id].type === "password" &&
                        data[tab][id] !== this.passwordRepeat[tab][id]
                    ) {
    ...
    getErrorData(validationResult) {
        if (
            !validationResult ||
            !(Symbol.iterator in Object(validationResult))
        ) {
            return {};
        }
        return formValidatorUtils.getErrorData(
            validationResult,
            window.__heretic.t,
        );
    }
    ...
    setLoading(flag) {
        ...
        document.getElementById(`${this.input.id}_form_body`).style.opacity =
            flag ? "0.3" : "1";
        return this;
    }
            ...
            if (!data[tab]) {
                ...
                for (const fieldId of this.fieldIds) {
                    if (this.fieldsFlat[fieldId].defaultValue) {
                        data[tab][fieldId] =
                            this.fieldsFlat[fieldId].defaultValue;
                    } else {
    ...
    serializeData() {
        ...
        for (const tab of this.state.tabs) {
            for (const fieldId of this.fieldIds) {
                if (
                    this.fieldsFlat[fieldId].type === "files" &&
                    data.formTabs[tab][fieldId] &&
                    data.formTabs[tab][fieldId].length
                ) {
                    for (
                        let i = 0;
                        i < data.formTabs[tab][fieldId].length;
                        i += 1
                    ) {
                        if (data.formTabs[tab][fieldId][i].data) {
                            data.upload[data.formTabs[tab][fieldId][i].uid] =
                                data.formTabs[tab][fieldId][i].data;
                        }
                        data.formTabs[tab][fieldId][i] = cloneDeep(
                            data.formTabs[tab][fieldId][i],
                        );
                        delete data.formTabs[tab][fieldId][i].data;
                    }
                }
                if (
                    this.fieldsFlat[fieldId].type === "keyValue" &&
                    Array.isArray(data.formTabs[tab][fieldId])
                ) {
                    for (
                        let i = 0;
                        i < data.formTabs[tab][fieldId].length;
        ...
        for (const sharedFieldId of this.sharedFieldIds) {
            for (const tab of this.state.tabs) {
                data.formShared[sharedFieldId] =
                    data.formTabs[tab][sharedFieldId];
    ...
    getFormDataObject(serializedData) {
        ...
        formData.append(
            "formShared",
            JSON.stringify(serializedData.formShared),
        );
    ...
    showNotification(message, css = "") {
        this.getComponent(`notify_field_${this.input.id}`).show(
    ...
    setErrorMessage(message) {
        ...
        if (message) {
            this.utils
                .waitForElement(`${this.input.id}_errorNotification`)
                .then(() => {
                    document
                        .getElementById(`${this.input.id}_errorNotification`)
                        .scrollIntoView({
                            block: "start",
                            inline: "nearest",
                        });
    ...
    async onModeChange(e) {
        e.preventDefault();
        const { mode } = e.target.closest("[data-mode]").dataset;
        this.switchMode(mode);
    }

    generateHistoryPagination() {
        const center = [
            this.state.historyPage - 2,
        ...
        const filteredCenter = center.filter(
            (p) => p > 1 && p < this.state.historyTotalPages,
        );
        // includeThreeLeft
    ...
    async setHistoryData(
        ...
        this.setState(
            "historyTotalPages",
            historyData.total < historyData.itemsPerPage
                ? 1
                : Math.ceil(historyData.total / historyData.itemsPerPage),
        );
        this.generateHistoryPagination();
        await this.utils.waitForComponent(`historyModal_hf_${this.input.id}`);
        const historyModal = this.getComponent(
            `historyModal_hf_${this.input.id}`,
        );
    ...
    async onHistoryActionDelete(e) {
        ...
        await this.utils.waitForComponent(
            `historyDeleteConfirmation_hf_${this.input.id}`,
        );
        this.getComponent(
            `historyDeleteConfirmation_hf_${this.input.id}`,
        ).setActive(true);
    ...
    async onKeyValueAddRequest(par) {
        ...
        const keyValueModal = this.getComponent(
            `keyValueModal_hf_${this.input.id}`,
        );
        ...
        await this.utils.waitForElement(
            `hform_keyValue_value_${this.input.id}`,
        );
        this.setKeyValueDefaults(this.state.keyValueData[0].id);
        document
            .getElementById(`hform_keyValueModal_${this.input.id}_key`)
            .focus();
    }
    ...
    async saveKeyValueFormData() {
        ...
        await this.utils.waitForComponent(
            `hr_hf_f_${this.state.keyValueFieldId}_${this.state.mode}`,
        );
        const fieldComponent = this.getComponent(
            `hr_hf_f_${this.state.keyValueFieldId}_${this.state.mode}`,
        );
        let valueLabel;
        const currentKeyValueItem = this.state.keyValueData.find(
            (i) => i.id === this.state.keyValueSelectedKey,
        );
        switch (currentKeyValueItem.type) {
            case "database":
            case "list":
                const valueItem = currentKeyValueItem.items.find(
                    (i) => i.id === this.state.keyValueValue,
                );
        ...
        const sameKeyValue = currentValue.find(
            (i) => i.id === this.state.keyValueSelectedKey,
        );
        if (
            currentKeyValueItem.unique &&
            ((!this.state.keyValueUID && sameKeyValue) ||
                (this.state.keyValueUID &&
                    sameKeyValue &&
                    sameKeyValue.uid !== this.state.keyValueUID))
        ) {
    ...
    setKeyValueDefaults(key) {
        const { type, items } = this.state.keyValueData.find(
            (i) => i.id === key,
        );
    ...
    async onKeyValueEditRequest(par) {
        await this.utils.waitForComponent(
            `hr_hf_f_${par.id}_${this.state.mode}`,
        );
        const fieldComponent = this.getComponent(
            `hr_hf_f_${par.id}_${this.state.mode}`,
        );
        const keyValueItem = fieldComponent
            .getValue()
            .find((i) => i.uid === par.uid);
        await this.utils.waitForComponent(`keyValueModal_hf_${this.input.id}`);
        const keyValueModal = this.getComponent(
            `keyValueModal_hf_${this.input.id}`,
        );
    ...
    async onKeyValueDeleteRequest(par) {
        await this.utils.waitForComponent(
            `groupDataDeleteConfirmation_hf_${this.input.id}`,
        );
        this.getComponent(
            `groupDataDeleteConfirmation_hf_${this.input.id}`,
        ).setActive(true);
    ...
    async onGroupDataDeleteConfirmationButtonClick(button) {
        switch (button) {
            case "delete":
                await this.utils.waitForComponent(
                    `hr_hf_f_${this.state.keyValueDeletePar.id}_${this.state.mode}`,
                );
                const fieldComponent = this.getComponent(
                    `hr_hf_f_${this.state.keyValueDeletePar.id}_${this.state.mode}`,
                );
                ...
                fieldComponent.setValue(
                    currentValue.filter(
                        (i) => i.uid !== this.state.keyValueDeletePar.uid,
                    ),
                );
                await this.utils.waitForComponent(
                    `groupDataDeleteConfirmation_hf_${this.input.id}`,
    ...
    async onTagClick(e) {
        ...
        await this.utils.waitForComponent(
            `hr_hf_f_${this.state.tagsData.id}_${this.state.mode}`,
        );
        const fieldComponent = this.getComponent(
            `hr_hf_f_${this.state.tagsData.id}_${this.state.mode}`,
        );
    ...
    onTagsFilterKeyup() {
        setTimeout(() => {
            const inputField = document.getElementById(
                `tagAddModal_filter_${this.input.id}`,
            );
    ...
    async showEmptyLogModal(par) {
        ...
        const logStatus = this.logFormData.data.form[0].fields.find(
            (i) => i.id === "logStatus",
        );
        ...
        const { validationData } = this.logFormData;
        ...
        const fieldComponent = this.getComponent(
            `hr_hf_f_${this.state.logFieldId}_${this.state.mode}`,
        );
        return {
            editForm,
            fieldComponent,
    ...
    async saveLogValue() {
        ...
        if (this.state.logValueUID) {
            const currentLogValueItem = currentLogValue.find(
                (i) => i.uid === this.state.logValueUID,
            );
            currentLogValueItem.logDate =
                serializedData.formTabs._default.logDate;
            currentLogValueItem.logValue =
                serializedData.formTabs._default.logValue;
            currentLogValueItem.logComments =
                serializedData.formTabs._default.logComments;
            currentLogValueItem.logStatus =
                serializedData.formTabs._default.logStatus;
        } else {
        ...
        await fieldComponent.setValue(
            currentLogValue.sort((a, b) =>
                a.logDate > b.logDate ? 1 : b.logDate > a.logDate ? -1 : 0,
            ),
        );
        await this.utils.waitForComponent(`logModal_hf_${this.input.id}`);
        const logModal = this.getComponent(`logModal_hf_${this.input.id}`);
    ...
    async onLogEditRequest(par) {
        const { fieldComponent, editForm } = await this.showEmptyLogModal(par);
        const currentValue = cloneDeep(fieldComponent.getValue()).find(
            (i) => i.uid === par.uid,
        );
    ...
    async onLogDeleteRequest(par) {
        await this.utils.waitForComponent(
            `logItemDeleteConfirmation_hf_${this.input.id}`,
        );
        this.getComponent(
            `logItemDeleteConfirmation_hf_${this.input.id}`,
        ).setActive(true);
    ...
    async onLogItemDeleteConfirmationButtonClick(button) {
        switch (button) {
            case "delete":
                await this.utils.waitForComponent(
                    `hr_hf_f_${this.state.logDeletePar.id}_${this.state.mode}`,
                );
                const fieldComponent = this.getComponent(
                    `hr_hf_f_${this.state.logDeletePar.id}_${this.state.mode}`,
                );
                ...
                fieldComponent.setValue(
                    currentValue.filter(
                        (i) => i.uid !== this.state.logDeletePar.uid,
                    ),
                );
                await this.utils.waitForComponent(
                    `logItemDeleteConfirmation_hf_${this.input.id}`,
