    ...
    async handler(req, rep) {
        try {
            ...
            if (
                ...
                Object.keys(this.languages).indexOf(language) < 0
            ) {
                return rep.error({
            ...
            limit =
                typeof limit === "string" && limit.match(/^[0-9]{1,3}$/)
                    ? limit
                    : "10";
            offset =
                typeof offset === "string" && offset.match(/^[0-9]{1,6}$/)
                    ? offset
                    : "0";
            ...
            let result = {};
            try {
                result = await this.redis.ft.search(
                    `${this.systemConfig.id}-fulltext`,
                    `@content|title:(${words}) @language:(${language.split(/-/)[0]})`,
                    {
