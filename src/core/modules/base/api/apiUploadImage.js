...
export default () => ({
    async handler(req, rep) {
        try {
            ...
            if (
                !multipartData.files ||
                !multipartData.files.image ||
                !multipartData.files.image.size >
                    moduleConfig.maxUploadImageSize
            ) {
                await req.removeMultipartTempFiles();
                return rep.error({
            ...
            const filename = this.systemConfig.demo
                ? ""
                : `${uuidv4()}${path.extname(multipartData.files.image.filename)}`;
            if (!this.systemConfig.demo) {
                await fs.ensureDir(path.resolve(__dirname, "public/images"));
                await fs.move(
                    multipartData.files.image.filePath,
