...
export default class {
    ...
    async loadData(dir = null) {
        ...
        data.append(
            "formTabs",
            `{"_default":{"dir":"${dir !== null ? dir : this.state.dir}"}}`,
        );
        ...
        try {
            ...
            disabled.paste =
                ...
                this.state.clipboard.src === this.state.dir;
    ...
    async onFileClick(e) {
        ...
        if (e.target.closest("[data-dropdown]")) {
            const { dropdown } = e.target.closest("[data-dropdown]").dataset;
            this.setState(
                "actionMenu",
                this.state.actionMenu === dropdown ? null : dropdown,
            );
            return;
        }
        ...
        if (e.target.closest("[data-id]")) {
            const { id } = e.target.closest("[data-id]").dataset;
            const { filename } = e.target.closest("[data-filename]").dataset;
            const fileData = this.state.files.find((f) => f.name === filename);
            const disabled = cloneDeep(this.state.disabled);
            switch (id) {
                case "download":
                    window.open(
                        `${this.systemRoutes.admin}/files/download?filename=${filename}&dir=${this.state.dir}&language=${this.language}`,
                        "_blank",
                    );
                    break;
                case "rename":
                    await this.utils.waitForComponent("nameModal");
                    this.getComponent("nameModal").show(
                        `${window.__heretic.t("rename")}: ${filename}`,
                        filename,
                        id,
                    );
                    break;
                case "edit":
                    ...
                    try {
                        const { data: editData } = await axios({
                            method: "post",
                            url: "/api/files/load",
                            data: dataEdit,
                            headers: {
                        ...
                        this.getComponent("editorModal").show(
                            filename,
                            editData.content,
                            fileData.mime,
                        );
                ...
                case "unzip":
                    ...
                    try {
                        const { data: processUnzipData } = await axios({
                            method: "post",
                ...
                case "untar":
                    ...
                    try {
                        const { data: processUntarData } = await axios({
                            method: "post",
                            url: "/api/files/process",
                            data: dataUntar,
                            headers: {
                ...
                case "untgz":
                    ...
                    try {
                        const { data: processUntgzData } = await axios({
                            method: "post",
                            url: "/api/files/process",
                            data: dataUntgz,
                            headers: {
    ...
    async onTopButtonClick(e) {
        ...
        switch (id) {
            ...
            case "paste":
                if (
                    !this.state.clipboard ||
                    !this.state.clipboard.files.length
                ) {
                    return;
                }
                this.setState("loading", true);
                ...
                data.append("tabs", `["_default"]`);
                try {
                    const { data: processData } = await axios({
                        method: "post",
                        url: "/api/files/process",
                        data,
                        headers: {
    ...
    async onFileSave(d) {
        ...
        try {
            ...
            await this.utils.waitForComponent("editorModal");
            const fileData = this.state.files.find(
                (f) => f.name === d.filename,
            );
            this.getComponent("editorModal").show(
                d.filename,
