...
export default () => ({
    async handler(req, rep) {
        try {
            ...
            const grandTotal = await this.mongo.db.collection(moduleConfig.collections.sessions).countDocuments({
                deleted: {
                    $exists: false,
                },
            });
            const total = await this.mongo.db.collection(moduleConfig.collections.sessions).countDocuments(query);
            const items = await this.mongo.db.collection(moduleConfig.collections.sessions).find(query, options).toArray();
            const usersQuery = [];
