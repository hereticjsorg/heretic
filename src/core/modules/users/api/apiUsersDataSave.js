...
export default () => ({
    async handler(req, rep) {
        ...
        try {
            ...
            const collection = this.mongo.db.collection(moduleConfig.collections.users);
            const result = {};
            ...
            if (data._id) {
                ...
                const duplicateErrors = await this.findDatabaseDuplicates(moduleConfig.collections.users, uniqueFields, data._default, existingRecord);
                if (duplicateErrors) {
                ...
                const duplicateErrors = await this.findDatabaseDuplicates(moduleConfig.collections.users, uniqueFields, data._default, null);
                if (duplicateErrors) {
                    return rep.error(duplicateErrors);
                }
                if (!this.systemConfig.demo) {
