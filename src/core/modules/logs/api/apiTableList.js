...
export default () => ({
    async handler(req, rep) {
        try {
            ...
            try {
                itemsAll = (
                    await ServerUtils.readLastStringsFromFile(
                        path.resolve(
                            __dirname,
                            `../logs/${ecosystem.logFileName}`,
                        ),
                        100000,
                    )
                )
                    ...
                    .map((i) => ({
                        ...
                        type: i.req ? "req" : "res",
                        code:
                            i.res && i.res.statusCode ? i.res.statusCode : null,
                        resTime: i.responseTime
                            ? parseFloat(i.responseTime).toFixed(3)
                            : null,
                        method: i.req && i.req.method ? i.req.method : null,
                        url: i.req && i.req.url ? i.req.url : null,
                        ip:
                            i.req && i.req.remoteAddress
                                ? i.req.remoteAddress
                                : null,
                        message: i.msg || null,
                    }));
                items = itemsAll.filter((i) => req.getFilterData(formData, i));
