                    ...
                    try {
                        if (req.body.profilePicture === "clear") {
                            ...
                            const [, profilePictureData] =
                                req.body.profilePicture.split(/base64,/); // cut data:image/png;base64,
                            const decodedBuffer = Buffer.from(
                                profilePictureData,
                                "base64",
                            );
                            const profilePictureBuffer = await sharp(
                                decodedBuffer,
                            )
                                .resize(
                                    moduleConfig.profilePicture.width,
                                    moduleConfig.profilePicture.height,
                                )
                                .jpeg({
                                    mozjpeg: true,
                                    quality: 70,
