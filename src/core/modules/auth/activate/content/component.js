...
export default class {
    ...
    onSignInButtonClick() {
        setTimeout(
            () =>
                (window.location.href = `${this.utils.getLocalizedURL(this.systemRoutes.signIn)}`),
        );
    }
    ...
    async onSetPasswordFormSubmit() {
        ...
        const validationResult = setPasswordForm.validate(
            setPasswordForm.saveView(),
        );
        if (validationResult) {
            return setPasswordForm.setErrors(
                setPasswordForm.getErrorData(validationResult),
            );
        }
        ...
        try {
            ...
            if (e && e.response && e.response.data) {
                if (e.response.data.form) {
                    const errorData = setPasswordForm.getErrorData(
                        e.response.data.form,
                    );
                    setPasswordForm.setErrors(errorData);
                }
                if (e.response.data.message) {
                    setPasswordForm.setErrorMessage(
                        this.t(e.response.data.message),
                    );
                } else {
                    setPasswordForm.setErrorMessage(
                        this.t("hform_error_general"),
                    );
                }
