...
export default class {
    ...
    async onRestorePasswordFormSubmit() {
        ...
        const validationResult = restorePasswordForm.validate(
            restorePasswordForm.saveView(),
        );
        if (validationResult) {
            return restorePasswordForm.setErrors(
                restorePasswordForm.getErrorData(validationResult),
            );
        }
        ...
        try {
            ...
            if (e && e.response && e.response.data) {
                if (e.response.data.form) {
                    const errorData = restorePasswordForm.getErrorData(
                        e.response.data.form,
                    );
                    restorePasswordForm.setErrors(errorData);
                }
                if (e.response.data.message) {
                    restorePasswordForm.setErrorMessage(
