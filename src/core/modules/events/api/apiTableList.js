...
export default () => ({
    async handler(req, rep) {
        try {
            ...
            const grandTotal = await this.mongo.db.collection(moduleConfig.collections.events).countDocuments({
                deleted: {
                    $exists: false,
                },
            });
            const total = await this.mongo.db.collection(moduleConfig.collections.events).countDocuments(query);
            const items = await this.mongo.db.collection(moduleConfig.collections.events).find(query, options).toArray();
            const usersQuery = [];
            const citiesQuery = [];
