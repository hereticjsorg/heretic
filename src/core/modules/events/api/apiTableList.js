    ...
    async handler(req, rep) {
        try {
            ...
            for (const item of items) {
                ...
                if (
                    item.geoNameIdCity &&
                    usersQuery.indexOf(item.geoNameIdCity) === -1
                ) {
                    citiesQuery.push(item.geoNameIdCity);
                }
                if (
                    item.geoNameIdCountry &&
                    countriesQuery.indexOf(item.geoNameIdCountry) === -1
                ) {
            ...
            if (citiesQuery.length) {
                ...
                citiesData = await this.mongo.db
            ...
            if (countriesQuery.length) {
                ...
                projection[req.body.language] = 1;
                countriesData = await this.mongo.db
            ...
            for (const item of items) {
                ...
                if (item.geoNameIdCountry) {
                    const countryData = countriesData.find(
                        (i) => String(i._id) === String(item.geoNameIdCountry),
                    );
                    if (countryData) {
                        ...
                        const defaultCountry =
                            countryData["en-us"] && countryData["en-us"].country
                ...
                if (item.geoNameIdCity) {
                    const cityData = citiesData.find(
                        (i) => String(i._id) === String(item.geoNameIdCity),
                    );
                    if (cityData) {
