    ...
    async handler(req, rep) {
        try {
            ...
            if (moduleConfig.recycleBin && moduleConfig.recycleBin.enabled && !this.systemConfig.demo) {
                const updateResult = await this.mongo.db.collection(moduleConfig.collections.events).updateMany(query, {
                    $set: {
                        deleted: {
                            date: new Date(),
            ...
            const deleteResult = this.systemConfig.demo ? {
                deletedCount: 0,
            } : await this.mongo.db.collection(moduleConfig.collections.events).deleteMany(query);
            return rep.code(200).send({
