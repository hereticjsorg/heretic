...
/**
   ...
   Fork of https://github.com/umanghome/swipe-listener

   Swipe-listener is a very minimal library that allows listening
   for swipe gesture on literally any DOM element. Once invoked
   with a DOM element, simply listen for swipe event and determine
   the direction with the directions object.
*/
...
const SwipeListener = function (element, options) {
    ...
    const _touchend = function (e) {
        ...
        const xs = x[0];
        const xe = x[x.length - 1]; // Start and end x-coords
        const ys = y[0];
        const ye = y[y.length - 1]; // Start and end y-coords
        ...
        if (
            ...
            const event = new CustomEvent("swipe", eventData);
            element.dispatchEvent(event);
        } else {
            const cancelEvent = new CustomEvent("swipecancel", {
                detail: {
                    touch,
                    target: e.target,
                    ...eventCoords,
    ...
    const _touchmove = function (e) {
        ...
        if (touches.length > 1) {
            ...
            const xe = touches[touches.length - 1].x;
            const ys = touches[0].y; // Start and end y-coords
            const ye = touches[touches.length - 1].y;
            const eventData = {
                detail: {
                    x: [xs, xe],
                    y: [ys, ye],
