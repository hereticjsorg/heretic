...
UnzipStream.prototype.processDataChunk = function (chunk) {
    ...
    switch (this.state) {
        ...
        case states.START:
            ...
            switch (signature) {
                ...
                default:
                    ...
                    if (
                        !isStreamStart &&
                        (signature & 0xffff) !== 0x4b50 &&
                        this.skippedBytes < 26
                    ) {
                    ...
                    var errMsg = isStreamStart
                        ? "Not a valid zip file"
                        : "Invalid signature in zip file";
                    if (this.options.debug) {
                        ...
                        console.log(
                            `Unexpected signature in zip file: 0x${sig.toString(16)}`,
                            `"${asString}", skipped`,
                            this.skippedBytes,
                            "bytes",
                        );
        ...
        case states.LOCAL_FILE_HEADER_SUFFIX:
            ...
            entry.path = this._decodeString(
                chunk.slice(0, this.parsedEntity.fileNameLength),
                isUtf8,
            );
            var extraDataBuffer = chunk.slice(
                this.parsedEntity.fileNameLength,
                this.parsedEntity.fileNameLength +
                    this.parsedEntity.extraFieldLength,
            );
            ...
            if (extra && extra.parsed) {
                ...
                if (
                    Number.isFinite(extra.parsed.uncompressedSize) &&
                    this.parsedEntity.uncompressedSize === FOUR_GIGS - 1
                ) {
                    this.parsedEntity.uncompressedSize =
                        extra.parsed.uncompressedSize;
                }
                if (
                    Number.isFinite(extra.parsed.compressedSize) &&
                    this.parsedEntity.compressedSize === FOUR_GIGS - 1
                ) {
                    this.parsedEntity.compressedSize =
                        extra.parsed.compressedSize;
                }
            }
        ...
        case states.CENTRAL_DIRECTORY_END_COMMENT:
            if (this.options.debug) {
                console.log(
                    ...
                    chunk.slice(0, requiredLength).toString(),
                );
            }
...
UnzipStream.prototype._prepareOutStream = function (vars, entry) {
    ...
    const isDirectory =
        vars.uncompressedSize === 0 && /[\/\\]$/.test(entry.path);
    ...
    if (!fileSizeKnown) {
        ...
        const matcherStream = new MatcherStream(
            searchPattern,
            (matchedChunk, sizeSoFar) => {
                ...
                if (
                    !zip64Mode &&
                    !compressedSizeMatches &&
                    sizeSoFar >= FOUR_GIGS
                ) {
                    let overflown = sizeSoFar - FOUR_GIGS;
                    while (overflown >= 0) {
                        compressedSizeMatches =
                            vars.compressedSize === overflown;
                ...
                if (self.data.length > 0) {
                    self.data = Buffer.concat([
                        matchedChunk.slice(sliceOffset),
                        self.data,
                    ]);
                } else {
        ...
        switch (vars.extraId) {
            ...
            case 0x5455:
                ...
                if (vars.extraSize >= offset + 4 && timestampFields & 1) {
                    extra.mtime = new Date(
                        data.readUInt32LE(index + offset) * 1000,
                    );
                    offset += 4;
                }
                if (vars.extraSize >= offset + 4 && timestampFields & 2) {
                    extra.atime = new Date(
                        data.readUInt32LE(index + offset) * 1000,
                    );
                    offset += 4;
                }
                if (vars.extraSize >= offset + 4 && timestampFields & 4) {
                    extra.ctime = new Date(
                        data.readUInt32LE(index + offset) * 1000,
                    );
                }
                break;
            ...
            case 0x756e:
                ...
                if (vars.extraSize >= 14) {
                    ...
                    if (vars.extraSize > 14) {
                        ...
                        const symlinkName = this._decodeString(
