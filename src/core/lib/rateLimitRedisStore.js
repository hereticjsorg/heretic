...
RedisStore.prototype.redisIncrement = async function (
    key,
    timeWindow,
    max,
    ban,
...
RedisStore.prototype.incr = function (ip, cb, max, ban) {
    this.redisIncrement(`${this.key}${ip}`, this.timeWindow, max, ban).then(
        (current) => cb(null, current),
    );
};
...
RedisStore.prototype.child = function (routeOptions) {
    return new RedisStore(
        ...
        `${this.redis.hereticId}${this.key}${routeOptions.routeInfo.method}${routeOptions.routeInfo.url}-`,
    );
};
