...
module.exports = class {
    ...
    log(message, options = {}) {
        ...
        if (options.noDate || this.logNoDate) {
            ...
            // eslint-disable-next-line no-console
            console.log(
                `${this.color.get(` ${currentDateTime} `, ["black", "bgWhite"])} ${text}`,
            );
        }
    }
    ...
    writeNavigationConfig() {
        ...
        const configNavigation = fs.readJSONSync(
            path.resolve(
                __dirname,
                "../../core/defaults/config/navigation.json",
            ),
        );
        if (
            !fs.existsSync(
                path.resolve(__dirname, "../../../site/modules/sample"),
            )
        ) {
    ...
    async createAdmin() {
        ...
        const resultUser = await this.db
        ...
        const resultGroup = await this.db
            .collection(this.config.collections.groups)
            .findOneAndUpdate(
                {
                    group: "admin",
                },
    ...
    async resetPassword(username) {
        ...
        if (resultUser && resultUser._id) {
            this.log(
                `User "${username}" has been created/updated in the database. New password is: password`,
            );
        } else {
    ...
    addModule(id, addNavigationConfig = false) {
        ...
        if (!id || !id.match(/^[a-z0-9_-]+$/i)) {
            this.log(
                "Invalid module ID, use latin characters, numbers and '-', '_' chars only",
                {
                    warning: true,
                },
            );
        ...
        if (
            fs.existsSync(
                path.resolve(__dirname, `../../../site/modules/${id}`),
            )
        ) {
        ...
        fs.copySync(
            path.resolve(__dirname, "../../core/defaults/modules/blank"),
            path.resolve(__dirname, `../../../site/modules/${id}`),
        );
        const moduleConfigPath = path.resolve(
            __dirname,
            `../../../site/modules/${id}/module.json`,
        );
        ...
        if (addNavigationConfig) {
            const navJSONPath = path.resolve(
                __dirname,
                "../../../site/etc/navigation.json",
            );
    ...
    removeModule(id) {
        ...
        if (navJSON.userspace.routes.indexOf(`${id}_page`) >= 0) {
            ...
            navJSON.userspace.routes = navJSON.userspace.routes.filter(
                (r) => r !== `${id}_page`,
            );
            navJSON.userspace.home =
                navJSON.home === `${id}_page` ? "" : navJSON.home;
            fs.writeJSONSync(navJSONPath, navJSON, {
                spaces: "  ",
            });
        }
    ...
    addLanguage(id, name) {
        ...
        const languageJSONPath = path.resolve(
            __dirname,
        ...
        const mainMetaJSONPath = path.resolve(
            __dirname,
            "../../../site/etc/meta.json",
        );
        const mainMetaJSON = fs.readJSONSync(mainMetaJSONPath);
        mainMetaJSON.title[id] =
            mainMetaJSON.title[Object.keys(languageJSON)[0]];
        mainMetaJSON.shortTitle[id] =
            mainMetaJSON.shortTitle[Object.keys(languageJSON)[0]];
        mainMetaJSON.description[id] =
            mainMetaJSON.description[Object.keys(languageJSON)[0]];
        ...
        for (const m of buildJson.modules) {
            if (m.translations) {
                const translationDataDefault = fs.readJSONSync(
                    path.resolve(
                        `${m.path}/translations/${Object.keys(languageJSON)[0]}.json`,
                    ),
                );
                fs.writeJSONSync(
                    path.resolve(`${m.path}/translations/${id}.json`),
                    translationDataDefault,
                    {
                        spaces: "  ",
                    },
                );
            ...
            for (const p of m.pages) {
                if (
                    !fs.existsSync(path.resolve(`${m.path}/${p.id}/meta.json`))
                ) {
                    continue;
                }
                const pageMetaJson = fs.readJSONSync(
                    path.resolve(`${m.path}/${p.id}/meta.json`),
                );
                if (pageMetaJson.title) {
                    pageMetaJson.title[id] =
                        pageMetaJson.title[Object.keys(languageJSON)[0]];
                }
                if (pageMetaJson.description) {
                    pageMetaJson.description[id] =
                        pageMetaJson.description[Object.keys(languageJSON)[0]];
                }
                fs.writeJSONSync(
                    path.resolve(`${m.path}/${p.id}/meta.json`),
                    pageMetaJson,
                ...
                if (
                    p.langSwitchComponent &&
                    !fs.existsSync(
                        path.resolve(`${m.path}/${p.id}/content/lang-${id}`),
                    )
                ) {
                    fs.ensureDirSync(
                        path.resolve(`${m.path}/${p.id}/content/lang-${id}`),
                    ...
                    fs.writeFileSync(
                        path.resolve(
                            `${m.path}/${p.id}/content/lang-${id}/index.marko`,
                        ),
                        `<div>${name}</div>`,
                    );
                }
        ...
        const transCoreJSON = fs.readJSONSync(
            path.resolve(
                __dirname,
                `../../translations/${Object.keys(languageJSON)[0]}.json`,
            ),
        );
        fs.writeJSONSync(
            path.resolve(__dirname, `../../translations/${id}.json`),
            transCoreJSON,
            {
                spaces: "  ",
            },
        ...
        if (
            fs.existsSync(path.resolve(__dirname, "../../../site/translations"))
        ) {
            this.log("Modifying site translation files...");
            const transUserJSON = fs.readJSONSync(
                path.resolve(
                    __dirname,
                    `../../../site/translations/${Object.keys(languageJSON)[0]}.json`,
                ),
            );
            fs.writeJSONSync(
                path.resolve(
                    __dirname,
                    `../../../site/translations/${id}.json`,
                ),
                transUserJSON,
                {
                    spaces: "  ",
    ...
    removeLanguage(id) {
        ...
        for (const m of buildJson.modules) {
            if (
                m.translations &&
                fs.existsSync(path.resolve(`${m.path}/translations/${id}.json`))
            ) {
                fs.removeSync(
                    path.resolve(`${m.path}/translations/${id}.json`),
                );
        ...
        this.log("Modifying core translation files...");
        const coreLangPath = path.resolve(
            __dirname,
        ...
        const userLangPath = path.resolve(
            __dirname,
            `../../../site/translations/${id}.json`,
        );
    ...
    async geoCleanUp() {
        ...
        this.log("Cleaning up...", {});
        await this.db
            .collection(this.config.collections.geoCountries)
            .deleteMany({});
        await this.db
            .collection(this.config.collections.geoNetworks)
            .deleteMany({});
        await this.db
            .collection(this.config.collections.geoCities)
            .deleteMany({});
    }
    ...
    async geoImportBlocksV4() {
        ...
        const { size: fileSize } = fs.statSync(
            `${__dirname}/../geo/geoNetworksV4.hgd`,
        );
        ...
        while (globalOffset <= fileSize) {
            ...
            while (pos < uncompressedBuf.length) {
                ...
                if (blockEnd) {
                    insertData.push({
                        geoNameIdCity: geoNameIdCity
                            ? parseInt(geoNameIdCity, 10)
                            : null,
                        geoNameIdCountry: geoNameIdCountry
                            ? parseInt(geoNameIdCountry, 10)
                            : null,
                        blockEnd,
                    });
                }
    ...
    async geoImportBlocksV6() {
        ...
        const { size: fileSize } = fs.statSync(
            `${__dirname}/../geo/geoNetworksV6.hgd`,
        );
    ...
    async geoImportCountries() {
        ...
        const { size: fileSize } = fs.statSync(
            `${__dirname}/../geo/geoCountries.hgd`,
        );
        ...
        while (globalOffset <= fileSize) {
            ...
            pos += 4;
            const continentCode = uncompressedBuf
                .subarray(pos, pos + 2)
                .toString();
            pos += 2;
            const countryCode = uncompressedBuf
                .subarray(pos, pos + 2)
                .toString();
            pos += 2;
            ...
            pos += 4;
            const strData = uncompressedBuf
                .subarray(pos, pos + strDataLen)
                .toString();
            const data = {};
        ...
        return new Promise((resolve, reject) => {
            data.pipe(unzip.Parse())
                .on("entry", (entry) => {
                    ...
                    const entryPathParsed = entryPath.replace(
                        /hereticjsorg-heretic-[a-z0-9]+\//,
                        "",
                    );
                    if (type === "Directory") {
            ...
            data.pipe(unzip.Parse())
                .on("entry", (entry) => {
                    ...
                    if (type === "Directory") {
                        ...
                        entry.pipe(
                            fs.createWriteStream(
                                path.join(dirPath, entryDirName, entryFileName),
                            ),
                        );
                    }
                })
    ...
    async patchPackageJson(dirPath) {
        const oldPackageJson = await fs.readJSON(
            path.join(__dirname, "../../../package.json"),
        );
        const newPackageJson = await fs.readJSON(
            path.join(dirPath, "root/package.json"),
        );
