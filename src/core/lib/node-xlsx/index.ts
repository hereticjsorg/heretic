...
export const parse = <T = any[]>(mixed: unknown, options: Sheet2JSONOpts & ParsingOptions = {}) => {
  const {
 dateNF, header = 1, range, blankrows, defval, raw = true, rawNumbers, ...otherOptions
} = options;
...
export const build = (
  ...
  {
 parseOptions = {}, writeOptions = {}, sheetOptions = {}, ...otherOptions
}: BuildOptions = {}
): Buffer => {
  const {
 bookType = "xlsx", bookSST = false, type = "buffer", ...otherWriteOptions
} = writeOptions;
 ...
 bookType, bookSST, type, ...otherWriteOptions
});
};
