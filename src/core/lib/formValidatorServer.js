...
export default class {
    ...
    validate() {
        ...
        const filesFields = Object.keys(this.fields).find(
            (k) => this.fields[k] && this.fields[k].type === "files",
        );
        for (const ff of Array.isArray(filesFields)
            ...
            for (const tab of this.tabs) {
                const filesData = this.data[tab][ff]
                    ? Array.isArray(this.data[tab][ff])
                        ? this.data[tab][ff]
                        : [this.data[tab][ff]]
                    : [];
                if (filesSchema) {
            ...
            for (const tab of this.tabs) {
                if (this.data[tab]) {
                    for (const k of Object.keys(this.data[tab])) {
                        if (this.fields[k]) {
                            this.data[tab][k] = this.processValue(
                                this.fields[k].type,
                                this.data[tab][k],
                            );
                        }
    ...
    async saveFiles(page = null, refId = null) {
        ...
        for (const ff of Array.isArray(filesFields)
            ...
            for (const tab of this.tabs) {
                ...
                for (const file of filesData) {
                    if (
                        this.formFiles[file.uid] &&
                        !this.fastify.systemConfig.demo
                    ) {
                        try {
                            await fs.move(
                                this.formFiles[file.uid].filePath,
                                `${path.resolve(__dirname, this.fastify.systemConfig.directories.files)}/${file.uid}`,
                            );
                            if (this.fastify.systemConfig.mongo.enabled) {
                                await this.fastify.mongo.db
                                    .collection(
                                        this.fastify.systemConfig.collections
                                            .files,
                                    )
                                    .insertOne({
                                        _id: file.uid,
                                        filename:
                                            this.formFiles[file.uid].filename,
                                        size: this.formFiles[file.uid].size,
                                        mimeType:
                                            this.formFiles[file.uid].mimeType,
                                        page,
                                        refId,
                                    });
                            }
    ...
    async unlinkRemovedFiles(currentItem) {
        ...
        for (const ff of Array.isArray(filesFields)
            ...
            : [filesFields]) {
            for (const tab of this.tabs) {
                const filesDataCurrent = currentItem[tab][ff]
                    ? Array.isArray(currentItem[tab][ff])
                        ? currentItem[tab][ff]
                        : [currentItem[tab][ff]]
                    : [];
                const filesDataNew = this.data[tab][ff]
                    ? Array.isArray(this.data[tab][ff])
                        ? this.data[tab][ff]
                        : [this.data[tab][ff]]
                    : [];
        ...
        for (const _id of removedUIDs) {
            ...
            try {
                await fs.unlink(
                    `${path.resolve(__dirname, this.fastify.systemConfig.directories.files)}/${_id}`,
                );
            } catch {
                // Ignore
            }
        ...
        if (this.fastify.systemConfig.mongo.enabled) {
            try {
                await this.fastify.mongo.db
                    .collection(this.fastify.systemConfig.collections.files)
