            ...
            return rep
                ...
                .redirect(
                    languages[0] === language
                        ? this.systemConfig.routes.admin
                        : `/${language}${this.systemConfig.routes.admin}`,
                );
        }
        if (page.routePath !== "/admin/signIn" && !authData) {
            return rep
                ...
                .redirect(
                    languages[0] === language
                        ? `${this.systemConfig.routes.signInAdmin}?r=${page.routePath}`
                        : `/${language}${this.systemConfig.routes.signInAdmin}?r=/${language}${page.routePath}`,
                );
        }
        if (
            page.routePath !== "/admin/signIn" &&
            (!authData ||
                !authData.groupData ||
            ...
            return rep
                .code(302)
                .redirect(languages[0] === language ? "/" : `/${language}`);
        }
