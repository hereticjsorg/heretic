...
export default class {
    ...
    async onMount() {
        ...
        if (!currentToken) {
            setTimeout(
                () =>
                    (window.location.href = `${this.getLocalizedURL(this.systemRoutes.signIn)}?r=${this.getLocalizedURL(moduleConfig.routes.userspace.list.path)}`),
                100,
            );
            return;
        }
        ...
        if (window.__heretic.routeExtra) {
            if (window.__heretic.routeExtra.success) {
                await this.utils.waitForComponent(
                    `notify_${moduleConfig.id}List`,
                );
                this.getComponent(`notify_${moduleConfig.id}List`).show(
    ...
    onUnauthorized() {
        ...
        setTimeout(
            () =>
                (window.location.href = `${this.getLocalizedURL(this.systemRoutes.signIn)}?_=${new Date().getTime()}&r=${this.getLocalizedURL(moduleConfig.routes.userspace.list.path)}`),
            100,
        );
    }
